...
const PaymentPage = () => {
  ...
  const getData = async () => {
    const response = await axios.get(
      `https://api-rent-car.vercel.app/order/${id}`
    );
  ...
  const handleSubmit = async (e) => {
    ...
    const post = await axios.patch(
      `https://api-rent-car.vercel.app/order/payment/${id}`,
      {
        ...
        {loading === false && datas != null && statusPayment === "pending" ? (
          <div className="flex flex-col lg:flex-row lg:items-center justify-center w-full py-2 pb-4">
            ...
            <div className="flex flex-col lg:w-1/2">
              {message !== "" && (
                <div
                  className={` transition-all duration-500 mb-2 w-full py-3 text-center text-black rounded-md ${
                    message === "" ? "hidden" : "block"
                  } ${status !== 200 ? "bg-red-200" : "bg-green-200"} `}
                  type="submit"
                >
                  {message}
                </div>
              ...
              <div className="flex flex-wrap justify-between items-center gap-4 ">
                ...
                >
                  ...
                  />
                  <button
                    className="bg-primary font-medium w-full py-3 px-4 text-center text-black rounded-md transition-all duration-150 focus:bg-amber-500"
                    type="submit"
                  >
                    pay
                  </button>
          ...
          <h1 className="text-2xl lg:text-4xl py-12 text-center font-semibold">
